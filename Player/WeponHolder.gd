...
@onready var weapon_list = ["Derringer"]
...
func spawn_new_gun(returned_index):
	...
	match weapon_list[returned_index]:
		"Derringer":
			...
			#updates the UI
			get_parent().get_node("Camera3D/PlayerUI/GunName").text="22 Popgun"
		...
		"Shotgun":
			get_parent().get_node("Camera3D/PlayerUI/GunName").text="Boomstick"
		...
		"Mac10":
			get_parent().get_node("Camera3D/PlayerUI/GunName").text="Shoot n' Scoot"
		...
		"Glock":
			get_parent().get_node("Camera3D/PlayerUI/GunName").text="Problem Solver"
